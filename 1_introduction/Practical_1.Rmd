...
```markdown
![](https://drive.google.com/file/d/1DjICv1pZ6is6o9ksny59Xlhh2bgdMr1h/view?usp=sharing)
...
# The variable y contains the mean of 5 values (x)
x<-c(1,2,3,4,5)
y<-mean(x)
```
...
`c(1, 2, 3, "Hi")`
The line above creates a vector of 4 categorical variables. If we run class() on the vector created it returns "character."


* Would the following be valid?

`c(1, 2, 3, "Hi") + 1 `
No, because "Hi" is non-numeric so 1 cannot be added to it.

* How about 

`c(1, 2, 3, FALSE) + 1 `
Yes. When I run class() on this vector it returns "numeric." Therefore, we can add a numeric value (1) to the vector.
...
```{r}
# Write you code in this cell
x_axis<-c(0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8,8.5,9,9.5,10)
```
...
```{r}
# Write you code in this cell
qplot(x=x_axis, y=y_axis,xlab="x axis", ylab="y axis", main="My Cool Plot",size=(0.5))

![](https://drive.google.com/file/d/1AwKfOO6kjWnPlhAODNvk4iK4kVHN8vmp/view?usp=sharing)
